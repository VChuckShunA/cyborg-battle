...
void Hero::dash(){
	if (hp > 0 && (state == HERO_STATE_MOVE || state == HERO_STATE_IDLE)) {
		...
		//push the hero in the direction in the direction they are travelling
		slideAngle = angle;
		slideAmount = 300;
		invincibleTimer = 0.1;
...
void Hero::changeAnimation(int newState, bool resetFrameToBegining){
	state = newState;
...
void Hero::updateAnimation(){
	...
	//if state says we're moving, but we're not, then change state/anim to idle
	...
	if (frameTimer >= currentFrame->duration) {
		...
		if (currentFrame->frameNumber == currentAnim->getEndFrameNumber()) {
			if (state == HERO_STATE_SLASH || state == HERO_STATE_DASH) {
				...
				//just reset animation (loops it back around ). THis is done for animations like Idle
				currentFrame = currentAnim->getFrame(0);
			}
		}
		else {
